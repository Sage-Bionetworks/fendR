...
#  sig.genes<-apply(all.pvals,2,function(x) length(which(p.adjust(x)<0.05)))
  ...
  if(file.exists(newf)){
   ...
   # print(v.res)
    pcsf.res.id <-fendR::runPcsfWithParams(ppi=combined.graph,terminals=abs(v.res),dummies=tested.drugs,w=w,b=b,mu=mu,doRand=TRUE)
...
#'@param thresholds
#'
trackNetworkStats<-function(pcsf.res.list,synTableId='syn12104377',esetFileId,viperFileId,thresholds){
  require(synapser)

  pcsf.parent='syn12104375'
...
eset.file='syn11912278'
viper.file='syn11910414'
#thresholds=c(0.25,0.75)
for(w in c(2,3,4,5)){
 for(b in c(1,2,5,10)){
